...
import olie_utilities as ouu

try:
    file = sys.argv[1]
    lat = ouu.parse_float_at(2)
    lon = ouu.parse_float_at(3)
...
def load_config() -> dict:
    ...
    with open('weathermaps.json', 'r', encoding='UTF-8') as f:
        return json.load(f)


config = load_config()
outfile = f"{config['imagePath']}/{label}.png"
